# Event Scraper Agent Spec

## Purpose
此 Capability 旨在自動化技術社群活動（如 Accupass、Facebook 等平台）的追蹤。系統會自動抓取目標平台的網頁內容，透過 LLM (Gemini API) 結構化地提取活動資訊，根據使用者訂閱的關鍵字進行配對、過濾重複活動，最後透過 SMTP 發送自訂 HTML 卡片式通知郵件給使用者，並記錄完整的執行日誌。
